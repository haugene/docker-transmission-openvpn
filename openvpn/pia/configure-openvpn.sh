...
CUSTOM_BUNDLE="/etc/openvpn/pia_custom_bundle.zip"
...
if [ -f "$CUSTOM_BUNDLE" ]; then
    echo "Found custom PIA bundle at $CUSTOM_BUNDLE — using that instead of downloading"
    unzip -qjo "$CUSTOM_BUNDLE" -d "$VPN_PROVIDER_HOME"
else
